    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('business-profiles.update', $profile->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-xs text-gray-500 mt-1">{!! __('business.google_review_link_help', ['tool_url' => route('tools.google-review-link-generator')]) !!}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="locale" name="locale" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            <option value="en" {{ old('locale', $profile->locale) === 'en' ? 'selected' : '' }}>English</option>
                            <option value="pl" {{ old('locale', $profile->locale) === 'pl' ? 'selected' : '' }}>Polski</option>
                        </select>
                    ...
                    <div class="flex items-center justify-end mt-4">
                        <a href="{{ route('business-profiles.show', $profile->id) }}" class="text-gray-600 hover:text-gray-900 mr-4">{{ __('common.cancel') }}</a>
